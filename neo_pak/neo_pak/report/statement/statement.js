...
frappe.query_reports["Statement"] = {
	"filters": [
		{
			"fieldname": "company",
			"label": __("Company"),
...
frappe.query_report.get_filters_html_for_print = function() {
	let applied_filters = this.get_filter_values();
	if(this.report_name == "Statement")
		["range1","range2","range3","range4","ageing_based_on"].forEach(function(item){ if(item in applied_filters) delete applied_filters[item]});
